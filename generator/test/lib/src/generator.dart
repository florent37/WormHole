  ...
  Code _generateToBody(MethodElement method, String methodName) {
    ...
    if (method.parameters.isNotEmpty) {
      if (method.parameters.length == 1) {
        ...
        return Block.of([Block.of(codes), generateToReturn(method, methodCall)]);
      }
    } else {
      ...
      if (isList) {
        ...
        if (type.typeArguments.length > 0) {
          ...
          if (isPrimitiveType(parameterizedType)) {
            ...
            return Block.of([
              Code("  final $variableName = ($mapName[\"$parameterName\"] as List)?.map((e) => e == null ? null : $parameterizedType.fromJson(Map<String, dynamic>.from(e as Map<dynamic, dynamic>)))?.toList();"),
            ]);
          }
        }
      }
